...
| S12 | **A second viewer mode, RT (real time): the device streams its samples over a Wi-Fi WebSocket and the viewer plots them live, one streamed data point per EDF+ sample. Wi-Fi is therefore compiled in — reopening the "no radios" choice — but streaming is OFF by default and per-device opt-in.** (decided 2026-08-09) | Two things drove it. **Clinically:** there is currently no way to confirm a montage is good until the night is over and the recording has been processed; a live view catches a detached electrode or an unplugged belt while it can still be fixed. **Electrically:** the AD8232 puts electrodes on the patient, so a USB tether to a mains-powered host is a leakage-current path — the existing serial link was the obvious transport and is the wrong one. A battery-powered device streaming over its own SoftAP keeps the patient galvanically isolated, which is *why* this reopens the Wi-Fi exclusion in [hardware](../knowledge/hardware.md) rather than working around it. The power cost is real and is contained rather than dismissed: the radio is off unless enabled per-device (NVS flag, set over the existing serial command channel by `ESP32-C6-heart-idf/tools/set_rt_stream.py`) and shuts down after 10 minutes with no client, so an unattended night keeps exactly today's profile. **S3/S5/S6 are untouched:** the stream is a second *read* path, not a second boundary — nothing is persisted, no derived value is computed, the EDF+ remains the raw anchor, and the viewer still never writes Zarr. Everything downstream of Python processing (PLMI, HRV, LM/PLM overlays, windowed metrics) is simply absent in RT rather than faked. Wire contract `protosom.rt/1.0.0`; producer `ESP32-C6-heart-idf/components/rt_stream`, consumer `src_web/src/rt_*.ts`. See [viewer § RT vs batch](../knowledge/viewer.md). |
...
| O3 | **RIP physical-range fix (device)** | Confirm/correct the physical range and units for the LDC1612 Thoracic + Abdomen RIP belts written by `logger.cpp`. The firmware currently works around the ±1,000,000 nH / 16-bit quantization on the **digital** side — dividing raw counts by per-channel constants (`THORACIC_DIVISOR 15`, `ABDOMEN_DIVISOR 11`, tuned on one night's swing) — without adjusting the declared physical range, so the header's affine map no longer yields nanohenries. Decide whether to fix the header, keep the divisors, or both. Affects EDF+ header scaling at the raw anchor. | Dmitry (device) |
