...
This is a proof-of-concept open-source software for a Polysomnography (PSG), or a sleep study.
It runs on a Raspberry Pi 5, for now, and collects these signals from a patient:
- Respiratory Inductance Plethysmography (RIP)
- Nasal air flow rate
- Heart rate and RR intervals
- Sound recording for snoring detection

And saves results to an industry standard EDF+ file, which can be viewed using excellent EDFBrowser by anyone, including any clinician or a specialist doctor.
...
----------------------------
- A high-quality USB microphone ( I use Mackie EM-91CU+ USB Condenser Microphone)
...
- Polar H9 heart rate sensor (moving to ECG Module AD8232 soon)

How to build
----------------------------
1. Clone the repo on a Raspberry Pi 5 (has to be 5, as it talks to the I2C bus in a new way)(ports to earlier Pi's welcome)

2. `cd ProtoSom`
3. `./setup_env.sh`

This will build everything and create a Python venv
...
How to run
----------------------------
Execute `./run_protosom.sh` (this relies on FFMpeg)

This will expect the hardware to be already connected, but will exit gracefully if not.
This will create a `tmux` session called `recording_session` because it's a long-running affair


Hardware
===========================
The goal in near future is to move to a custom, open source hardware, based on an ESP32 C6 microcontroller.
This is under active development in Kicad 10

It lives in the sister repo here:
https://github.com/sunrise-designs/OpenPolysom
